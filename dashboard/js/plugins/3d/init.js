var demo = {
	...
	init: function(htmlElementId,json,dw,dh,position,tooltip){
		...
		setTimeout(function(){
			...
			var parent = $("#"+htmlElementId).parent();
			console.log(parent.width(),parent.height());
			network.adjustBounds( $("#"+htmlElementId).parent().width(), $("#"+htmlElementId).parent().parent().height()-50);
		...
		dw = dw || 0;
		dh = dh || 0;
		// mono.Utils.autoAdjustNetworkBounds(network, document.documentElement, 'clientWidth', 'clientHeight', dw, dh + 46);
		tooltip = tooltip || new Tooltip(['设备名：'], ['000000']);
		document.body.appendChild(tooltip.getView());
		network.getRootView().addEventListener('click', function(e) {
			demo.handleClick(e, network, tooltip)
		});
		...
		demo.loadData(network, json);
		var time2 = new Date().getTime();
		console.log('time:  ' + (time2 - time1));
		demo.resetCamera(network, position)
	},
	resetCamera: function(network, position) {
		network.getCamera().setPosition(position.w, position.h, position.d);
	...
	setupLights: function(box) {
		var pointLight = new mono.PointLight(0xFF0000, 1.5);
		pointLight.setPosition(100, 100, 100);
		box.add(pointLight);
		var pointLight = new mono.PointLight(0x00FF00, 1.3);
	...
	handleDoubleClick: function(e, network) {
		...
		if (firstClickObject) {
			...
			console.log(element.getClient('animation'));
	...
	handleMouseMove: function(e, network, tooltipObj) {
		...
		if (objects.length) {
			var first = objects[0];
			var object3d = first.element;
			if (object3d.getClient('isTooltip')) {
				currentElement = object3d;
				tooltipObj.setValues([object3d.getClient('name')]);
			}
		}
	...
	copyProperties: function(from, to, ignores) {
		if (from && to) {
			for (var name in from) {
				if (ignores && ignores.indexOf(name) >= 0) {} else {
					to[name] = from[name]
				}
			}
		}
	...
	loadData: function(network, dataJson) {
			...
			if (type === 'rack' || type === 'rack.door') {
				...
				if (type === 'rack') {
					...
					for (var i = 0; i < element.spaceCubes.length; i++) {
						element.spaceCubes[i].setPosition(element.getPositionX(), element.spaceCubes[i].getPositionY(), element.getPositionZ());
