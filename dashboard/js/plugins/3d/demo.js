var demo={
	...
	init: function(htmlElementId,json,dw,dh,position,tooltip){				
		...
		tooltip = tooltip || new Tooltip(['设备名：','设备号：'],['000000','000000']);
	...
	handleClick: function(e, network ,tooltipObj){
		...
		if(objects.length > 0){
			...
			var name = object3d.getClient('name');
    ...
    handleMouseMove: function(e, network, tooltipObj){ 
        ...
        if (objects.length) {           
            ...
            if(object3d.getClient('isTooltip')){ 
                ...
                tooltipObj.setValues([object3d.getClient('name'),object3d.getClient('id')]);
            }
        }
	...
	createShadowImage: function(box, floorWidth, floorHeight){			
		...
		// marker(context, '刀弟云', '192.168.1.100', 530, 500);
		// marker(context, '魏蜀吴引擎', '192.168.1.150', 590, 1000);
		// marker(context, '通用厂房', 'ip待分配', 1020, 1000);
